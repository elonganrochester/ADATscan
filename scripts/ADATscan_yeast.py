#!/usr/bin/env python3
"""
python ADATscan_yeast.py <fasta> <null model output> <Enrichment and Depletion output> <gene body output>
...
codon_set = ['TCC','CCC','CGC','CGA','ATC','ACC','GTC','GCC'] #This set follows Percudanietal1997
##################STEP 1 GENERATE DATA STRUCTURES############################## 
...
for line in hexome:
	...
	for i in range(int((len(line_list[0]))/3)): #iterate through discreet windows of three nucleotides 
		...
		if current_codon == 'CGA' : radatreqcount +=1
...
for line in hexome: #for line in fasta file 
	...
	chisquareexp[0] = (tcount*tfrac)+(acount*afrac)+(pcount*pfrac)+(scount*sfrac)+(icount*ifrac)+(vcount*vfrac)+(rcount*rfrac) #calculate expected number based on TAPSIVR content and genomic background 
	######Calculate the expected number of adat dependent codons##########################
