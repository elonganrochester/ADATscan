#!/usr/bin/env python3
"""
python ADATscan_celegans.py  <fasta> <null model output> <Enrichment and Depletion output> <gene body output> 
...
for line in hexome:
	...
	if line_list[0][0] == '>': # if the line is a header line record the gene name 
		if line_list[0][1:][-1:] == 'b':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue
		if line_list[0][1:][-1:] == 'c':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'd':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue  
		if line_list[0][1:][-1:] == 'e':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'f':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'g':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'h':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'i':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'j':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'k':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'l':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'm':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'n':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'o':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'p':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'q':#filter out isoforms so same codons arent counted twice 
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'r':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 's':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 't':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'u':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'v':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'w':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'x':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue 
		if line_list[0][1:][-1:] == 'y':#filter out isoforms so same codons arent counted twice
			skipped = True 
			continue
		if line_list[0][1:][-1:] == 'z':#filter out isoforms so same codons arent counted twice
			xx += 1
			skipped = True 
			continue  
